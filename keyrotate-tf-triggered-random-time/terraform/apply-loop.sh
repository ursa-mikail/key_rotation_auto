...
while true; do
  if [ -f "$VARS_FILE" ]; then
    ...
    if terraform -chdir=/work apply -auto-approve -var-file="$VARS_FILE" -var="now=$NOW"; then
      ...
      if [ "$DUE" = "true" ]; then
        LABEL=$(terraform -chdir=/work output -raw primary_key_label 2>/dev/null || echo "unknown")
        log "trigger fired this cycle (new primary key label: $LABEL) -- see /shared/last-trigger-response.json for what Go returned"
      fi
    else
