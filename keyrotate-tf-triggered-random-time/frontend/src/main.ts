  ...
  <div class="wrap">
    ...
    <section class="card">
      ...
      <div class="status-grid">
        ...
        <div><span class="label">Rotation range</span><span id="s-interval" class="value">—</span></div>
        <div><span class="label">This cycle's expiry</span><span id="s-expires" class="value mono">—</span></div>
    ...
    <section class="card">
      ...
      <div id="tab-resource" class="tab-panel active">
        ...
        <p class="muted small">
          This is what the actual resource looks like right now, with live values plugged in —
          not the static <code>main.tf</code> source (that only has variable declarations and a
          commented-out example), and not raw JSON either. Regenerated fresh from the current key
          list on every update below.
        </p>
...
// Once a primary key exists anywhere in the system, generating another
// genesis key is never a valid action again -- there is exactly one
// genesis event, ever. This flag permanently locks the button; it is
// not reset by anything in this browser tab, only by render() seeing
// status.primaryKeyId become non-null (which happens from Postgres,
// not from the click handler's own success path -- see comment below).
...
genBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // Do NOT unconditionally re-enable here. If this request just
    // created (or discovered) a primary key, the very next status
    // tick will set genesisLocked = true and disable the button from
    // render() instead -- that's the authoritative path. Re-enabling
    // unconditionally in this finally block is exactly what let a
    // rapid re-click through before: the button was only ever guarded
    // by its own in-flight request, never by whether genesis had
    // actually happened.
...
function render(status: StatusResponse) {
  ...
  // own request succeed." This is what makes it safe against two
  // browser tabs, a rapid double-click, or this tab's own request
  // still being in flight when another one already created the key --
  // whichever request the backend actually accepted, every tab
  // converges to "locked" on its next status tick regardless.
  if (status.primaryKeyId && !genesisLocked) {
    genesisLocked = true;
    genBtn.disabled = true;
    genBtn.textContent = 'Genesis key already created';
  }
  ...
  document.querySelector('#s-interval')!.textContent = `${status.minSeconds}\u2013${status.maxSeconds}s (random)`;
  document.querySelector('#s-expires')!.textContent = status.expiresAt
  ...
  if (status.terraformInSync === undefined) {
    ...
    syncBadge.textContent = 'in sync — output file matches the current expiry';
  } else {
    // Expected for most of any given cycle in this variant: the output
    // file only catches up to the newest expires_at on the NEXT apply
    // after Go rotates, not the same one that triggered it. See
  ...
  // Go serializes a nil slice as `null`, not `[]` -- guard here even
  // though the backend now also avoids sending null (see controls.go,
  // listHistory/listGenesisAttempts) so a future response shape change
  // on either side can't reintroduce this as a crash-that-looks-like-
  // "backend unreachable" (a render() exception thrown inside the
  // polling loop's try block gets miscounted as a fetch failure).
...
//
// Deliberately two separate try/catch blocks, not one: a failure in
// fetchStatus() (the network call) and a failure in render() (pure
// DOM/JS, no network involved) are different problems with different
// causes, and conflating them is exactly what turned a null-vs-[]
// JSON shape bug into a misleading "backend unreachable" message
// during development -- the fetch was succeeding the entire time; a
// render() exception was being miscounted as a connection failure.
let pollFailures = 0;
setInterval(async () => {
  let status;
  try {
  ...
  try {
    ...
    // The fetch worked -- this is a rendering bug, not a connectivity
